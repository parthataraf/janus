...
through **vector + full-text retrieval** — one router picks the right path per
...
1. **Provision** the cheapest Hetzner/DigitalOcean VPS (2 vCPU / 4 GB is
   comfortable — the reranker is a CPU model; embeddings are hosted). Ubuntu 22.04+.
...
7. Open `https://<DOMAIN>` and ask a question. Firewall: open **80 and 443**
   ...
   OP.GG stats path) and to the generation endpoint — allow outbound 443. On
...
- **Structured *and* vector routing** — game data has exact numbers prose
  ...
  tables while mechanics/lore go through hybrid retrieval — both can feed one
  generation call.
...
stack — hybrid (dense + full-text, RRF) + cross-encoder reranking with an
